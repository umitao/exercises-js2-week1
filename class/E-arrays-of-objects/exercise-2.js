...
function isWithin500Kms(distance) {
...
function isReachableByFerry(transport) {
  return transport.transportations.includes("ferry");
}

var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(moreThan300Kms)
  .map(getNames);

function moreThan300Kms(destinations) {
  return (
    destinations.transportations.includes("train") &&
    destinations.distanceKms > 300
  );
}
